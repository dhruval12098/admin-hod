...
export default function CategoryDetailPage({ params }: CategoryDetailPageProps) {
  ...
  useEffect(() => {
    void params.then((resolved) => setCategorySlug(resolved.slug))
  }, [params])

  useEffect(() => {
    if (!categorySlug) return
    void loadData(categorySlug)
  }, [categorySlug])
  ...
  const loadData = async (slug: string) => {
    ...
    try {
      ...
      const nextCategory = categories.find((item) => item.slug === slug) ?? null
      ...
      setOptions(allOptions.filter((item) => nextSubcategories.some((subcategory) => subcategory.id === item.subcategory_id)))
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return <div className="p-8"><div className="rounded-lg border border-border bg-white px-6 py-12 text-sm text-muted-foreground">Loading category details...</div></div>
  }
    ...
    <div className="p-8">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as CatalogDetailTab)} className="w-full">
        ...
        <TabsContent value="subcategories" className="mt-6">
          <CategorySubcategoriesPanel category={category} subcategories={subcategories} onChange={() => loadData(category.slug)} />
        </TabsContent>

        <TabsContent value="options" className="mt-6">
          <CategoryOptionsPanel category={category} subcategories={subcategories} options={options} onChange={() => loadData(category.slug)} />
        </TabsContent>
      </Tabs>
    </div>
  )
...
function CategorySubcategoriesPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog isOpen={Boolean(deleteTarget)} title="Delete subcategory?" description={`Are you sure you want to delete "${deleteTarget?.name ?? ''}"?`} confirmText="Delete" type="delete" onConfirm={() => deleteTarget && void deleteItem(deleteTarget.id)} onCancel={() => setDeleteTarget(null)} />
    </div>
  )
}
...
function CategoryOptionsPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog isOpen={Boolean(deleteTarget)} title="Delete option?" description={`Are you sure you want to delete "${deleteTarget?.name ?? ''}"?`} confirmText="Delete" type="delete" onConfirm={() => deleteTarget && void deleteItem(deleteTarget.id)} onCancel={() => setDeleteTarget(null)} />
    </div>
  )
}
