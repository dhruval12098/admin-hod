...
export default function HipHopProductsPage() {
  ...
  const hiphopProducts = useMemo(
    () =>
      products.filter((product) => {
        const source = `${product.mainCategorySlug ?? ''} ${product.mainCategoryName ?? ''} ${product.categoryPath ?? ''}`.toLowerCase()
        return product.detailTemplate === 'hiphop' || source.includes('hip')
      }),
    [products]
  )
  ...
  return (
    <div className="p-8">
      <div className="mb-10 flex items-center justify-between">
        <div>
          <h1 className="font-jakarta text-3xl font-semibold text-foreground">Hip Hop Products</h1>
          <p className="mt-1 text-sm text-muted-foreground">Create and manage premium Hip Hop products with the dedicated template flow.</p>
        </div>
        <Link
          href="/dashboard/hiphop-products/create"
      ...
      <div className="mb-8">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search Hip Hop products..."
            value={search}
                ...
                <tr key={product.id} className="border-b border-border hover:bg-secondary/30 transition-colors duration-150">
                  ...
                  <td className="px-6 py-3.5">
                    <div className="flex items-center gap-1.5">
                      <Link href={`/dashboard/hiphop-products/edit/${product.slug}`} className="rounded p-1.5 hover:bg-secondary transition-colors" title="Edit">
      ...
      {loading ? <div className="mt-8 text-sm text-muted-foreground">Loading Hip Hop products...</div> : null}
      {!loading && filteredProducts.length === 0 ? (
        <div className="mt-12 text-center">
          <p className="text-sm text-muted-foreground">No Hip Hop products found.</p>
        </div>
      ) : null}

      <ConfirmDialog
        isOpen={Boolean(deleteTarget)}
        title="Delete this Hip Hop product?"
