...
export default function MaterialStripEditor() {
  const { toast } = useToast()
  const [items, setItems] = useState<MaterialItem[]>([])
  const [loadStatus, setLoadStatus] = useState('Loading Material Strip...')
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!response.ok) {
        setLoadStatus(payload?.error ?? 'Unable to load Material Strip.')
        return
      }

      setItems((payload?.items ?? []).map((item) => ({ clientId: `id-${item.id ?? item.sort_order}`, ...item })))
      setLoadStatus(payload?.items?.length ? 'Material Strip loaded' : 'No material strip rows found yet')
    }
