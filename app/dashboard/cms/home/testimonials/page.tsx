...
export default function TestimonialsEditor() {
  ...
  const [title, setTitle] = useState('Loved by Clients Worldwide')
  const [testimonials, setTestimonials] = useState<TestimonialItem[]>([])
  const [isSaving, setIsSaving] = useState(false)
  const [confirmOpen, setConfirmOpen] = useState(false)
  const [loadStatus, setLoadStatus] = useState('Loading testimonials...')
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!response.ok) {
        setLoadStatus(payload?.error ?? 'Unable to load testimonials.')
        return
      }
      ...
      setLoadStatus(items.length ? 'Testimonials loaded' : 'No testimonial items found yet')
    }
