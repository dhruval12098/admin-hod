...
export default function HomeBestSellersEditor() {
  const { toast } = useToast()
  const [form, setForm] = useState<EditorState>(defaultState)
  const [products, setProducts] = useState<ProductListItem[]>([])
  const [search, setSearch] = useState('')
  const [status, setStatus] = useState('Loading best sellers...')
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [sectionResponse, productsResponse] = await Promise.all([
        fetch('/api/cms/home/bestsellers', {
      ...
      const sectionPayload = await sectionResponse.json().catch(() => null)
      const productsPayload = await productsResponse.json().catch(() => null)

      if (!sectionResponse.ok) {
        setStatus(sectionPayload?.error ?? 'Unable to load best sellers section.')
        return
      }

      if (!productsResponse.ok) {
        setStatus(productsPayload?.error ?? 'Unable to load products.')
        return
      }

      setForm({
        eyebrow: sectionPayload?.section?.eyebrow ?? defaultState.eyebrow,
        heading: sectionPayload?.section?.heading ?? defaultState.heading,
        cta_label: sectionPayload?.section?.cta_label ?? defaultState.cta_label,
        cta_href: sectionPayload?.section?.cta_href ?? defaultState.cta_href,
        selected_product_ids: sectionPayload?.selected_product_ids ?? [],
      })
      setProducts(productsPayload?.items ?? [])
      setStatus('Best sellers loaded')
    }
