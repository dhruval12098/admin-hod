...
export function StatsEditorClient({ initialData }: { initialData: StatsInitialData }) {
  const { toast } = useToast()
  const [stats, setStats] = useState<StatItem[]>(
    initialData.items.map((item) => ({ clientId: `id-${item.id ?? item.sort_order}`, ...item }))
  )
  const [loadStatus, setLoadStatus] = useState(initialData.items.length ? 'Statistics Strip loaded' : 'No statistics rows found yet')
