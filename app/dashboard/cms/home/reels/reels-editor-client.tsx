...
export function ReelsEditorClient({ initialData }: { initialData: ReelsInitialData }) {
  ...
  const addDraftToList = () => {
    ...
    if (!isValidCoverImageUrl(draft.cover_image_url)) { setDraftError('Cover image must be a valid public image URL.'); return }
  ...
  const save = async () => {
    ...
    if (ordered.some((item) => !isValidCoverImageUrl(item.cover_image_url))) { setError('Cover image must be a valid public image URL.'); return }
    ...
    const response = await fetch('/api/cms/home/reels', {
      ...
      body: JSON.stringify({
        ...
        items: ordered.map(({ id, instagram_url, title, cover_image_url, is_enabled }) => ({ id, instagram_url, title, cover_image_url, is_enabled })),
      }),
    })
  ...
  return <main className="p-8">
    ...
    <header className="mb-8"><p className="text-xs font-semibold uppercase tracking-[.2em] text-primary">Homepage / Social</p><h1 className="mt-2 font-jakarta text-3xl font-semibold">Instagram Reels</h1><p className="mt-1 text-sm text-muted-foreground">Curate up to 30 public Instagram posts for the homepage marquee. Add a cover image URL for the card preview.</p></header>
    ...
    <section className="max-w-6xl overflow-hidden rounded-lg border bg-white shadow-xs">
      ...
      {ordered.length === 0 ? <p className="px-5 py-12 text-center text-sm text-muted-foreground">No reels yet. Add a public Instagram URL to begin.</p> : <div className="divide-y">{ordered.map((item,index)=><article key={item.clientId} className="grid gap-4 p-5 xl:grid-cols-[auto_minmax(280px,1fr)_minmax(220px,.75fr)_minmax(240px,.9fr)_auto] xl:items-end">
        ...
        <label className="text-xs font-semibold uppercase tracking-wider">Cover image URL<input value={item.cover_image_url} onChange={(e)=>update(item.clientId,{cover_image_url:e.target.value})} placeholder="https://.../cover.webp" className="mt-2 w-full rounded-md border px-3 py-2 text-sm font-normal normal-case tracking-normal"/></label>
    ...
    {addOpen ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-4 py-6" role="dialog" aria-modal="true" aria-labelledby="add-reel-title">
      <div className="w-full max-w-2xl rounded-lg border bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b px-6 py-5"><div><h2 id="add-reel-title" className="text-xl font-semibold">Add Instagram Reel</h2><p className="mt-1 text-sm text-muted-foreground">Paste a public Instagram URL and an optional cover image URL.</p></div><button type="button" onClick={closeAddDialog} className="rounded border p-2 text-muted-foreground hover:text-foreground" aria-label="Close add reel dialog"><X size={16}/></button></div>
        <div className="space-y-5 px-6 py-5">
          ...
          <label className="block text-sm font-semibold">Cover image URL<input value={draft.cover_image_url} onChange={(e)=>setDraft((current)=>({...current, cover_image_url:e.target.value}))} placeholder="https://.../cover.webp" className="mt-2 w-full rounded-md border px-3 py-2 font-normal"/></label>
