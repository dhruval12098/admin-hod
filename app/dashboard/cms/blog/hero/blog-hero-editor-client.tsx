...
export function BlogHeroEditorClient({ initialData }: { initialData: BlogHeroForm }) {
  ...
  const save = async () => {
    ...
    if (!accessToken) return setStatus('You are not signed in.')
    setIsSaving(true)
    const response = await fetch('/api/cms/blog/hero', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${accessToken}` }, body: JSON.stringify(form) })
  ...
  return <div className="min-h-screen bg-background p-8">
    ...
    <div className="grid max-w-5xl gap-6 lg:grid-cols-2">
      <section className="space-y-5 rounded-lg border border-border bg-white p-6 shadow-xs">
        <label className="flex items-center gap-3 text-sm font-semibold"><input type="checkbox" checked={form.is_enabled} onChange={(e) => setForm({ ...form, is_enabled: e.target.checked })} className="h-4 w-4" />Show hero on blog page</label>
