...
export async function GET(request: Request) {
  ...
  const { data, error } = await adminClient.from('bespoke_process_cards').select('id, sort_order, eyebrow, title, description').order('sort_order', { ascending: true })
...
export async function POST(request: Request) {
  ...
  const { adminClient } = access
  const { error: deleteError } = await adminClient.from('bespoke_process_cards').delete().gte('sort_order', 0)
  if (deleteError) return NextResponse.json({ error: deleteError.message }, { status: 500 })
  const rows = body.items
    .filter((item: any) => typeof item.eyebrow === 'string' && typeof item.title === 'string' && typeof item.description === 'string')
  ...
  if (rows.length > 0) {
    const { error: insertError } = await adminClient.from('bespoke_process_cards').insert(rows)
