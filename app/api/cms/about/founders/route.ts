...
export async function POST(request: Request) {
  ...
  const { adminClient } = access
  const { error: deleteError } = await adminClient.from('about_founders').delete().gte('sort_order', 0)
  ...
  if (rows.length > 0) {
    const { error: insertError } = await adminClient.from('about_founders').insert(rows)
