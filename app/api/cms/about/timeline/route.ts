...
export async function GET(request: Request) {
  ...
  const { data, error } = await adminClient.from('about_timeline').select('id, sort_order, year, label').order('sort_order', { ascending: true })
...
export async function POST(request: Request) {
  ...
  const { adminClient } = access
  const { error: deleteError } = await adminClient.from('about_timeline').delete().gte('sort_order', 0)
  if (deleteError) return NextResponse.json({ error: deleteError.message }, { status: 500 })

  const rows = body.items
    .filter((item: any) => typeof item.year === 'string' && typeof item.label === 'string')
  ...
  if (rows.length > 0) {
    const { error: insertError } = await adminClient.from('about_timeline').insert(rows)
