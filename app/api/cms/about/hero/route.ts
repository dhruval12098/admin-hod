...
export async function GET(request: Request) {
  ...
  return NextResponse.json(data ?? { section_key: sectionKey, eyebrow: 'Our Story', heading: 'Two Friends. One Vision.', subtitle: 'House of Diams was born in Surat, India.' })
}
...
export async function POST(request: Request) {
  ...
  if (!body || typeof body.eyebrow !== 'string' || typeof body.heading !== 'string' || typeof body.subtitle !== 'string') {
    return NextResponse.json({ error: 'Invalid payload.' }, { status: 400 })
  }
  const { adminClient } = access
  const { error } = await adminClient
    .from('about_hero')
    .upsert({ section_key: sectionKey, eyebrow: body.eyebrow, heading: body.heading, subtitle: body.subtitle }, { onConflict: 'section_key' })
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
