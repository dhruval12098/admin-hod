...
export async function GET(request: Request) {
  ...
  const { data, error } = await access.adminClient.from('blog_page_hero').select(fields).eq('id', 1).maybeSingle()
...
export async function POST(request: Request) {
  ...
  const { error } = await access.adminClient.from('blog_page_hero').upsert(payload, { onConflict: 'id' })
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
