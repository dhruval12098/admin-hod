...
export async function POST(request: Request) {
  ...
  const fileName = isSvg ? `material-strip/${crypto.randomUUID()}.svg` : `material-strip/${crypto.randomUUID()}.webp`
  const uploadBuffer = isSvg
    ? buffer
    : await sharp(buffer).rotate().resize({ width: 800, withoutEnlargement: true }).webp({ quality: 82 }).toBuffer()
