...
export async function POST(request: Request) {
  ...
  if (!allowed.has(contentType)) return NextResponse.json({ error: 'Invalid material image type.' }, { status: 400 })
  const path = `material-strip/${crypto.randomUUID()}.${contentType === 'image/svg+xml' ? 'svg' : 'webp'}`
