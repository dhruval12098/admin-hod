...
export async function POST(request: Request) {
  ...
  if (!allowedMimeTypes.has(contentType)) return NextResponse.json({ error: 'Invalid direct promotion image upload type.' }, { status: 400 })
  const extension = contentType === 'image/svg+xml' ? 'svg' : 'webp'
  const path = `promotion-popup/${crypto.randomUUID()}.${extension}`
  const { data, error } = await access.adminClient.storage.from(bucket).createSignedUploadUrl(path)
  if (error || !data?.token) return NextResponse.json({ error: error?.message ?? 'Unable to prepare direct promotion upload.' }, { status: 500 })
