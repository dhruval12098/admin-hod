...
export async function POST(request: Request) {
  ...
  if (!allowedMimeTypes.has(contentType)) {
    return NextResponse.json({ error: 'Invalid direct founder image upload type.' }, { status: 400 })
  }

  const extension = contentType === 'image/svg+xml' ? 'svg' : 'webp'
  const path = `founders/${crypto.randomUUID()}.${extension}`
  const { data, error } = await access.adminClient.storage.from(bucket).createSignedUploadUrl(path)
  if (error || !data?.token) {
    return NextResponse.json({ error: error?.message ?? 'Unable to prepare direct founder upload.' }, { status: 500 })
  }
