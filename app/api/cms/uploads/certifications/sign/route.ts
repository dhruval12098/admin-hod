...
export async function POST(request: Request) {
  ...
  if (!allowed.has(contentType)) return NextResponse.json({ error: 'Invalid certification image type.' }, { status: 400 })
  const path = `certifications/${crypto.randomUUID()}.${contentType === 'image/svg+xml' ? 'svg' : 'webp'}`
