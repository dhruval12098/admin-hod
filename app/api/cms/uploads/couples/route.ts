...
export async function POST(request: Request) {
  ...
  const fileName = isSvg ? `couples/${crypto.randomUUID()}.svg` : `couples/${crypto.randomUUID()}.webp`
  const uploadBuffer = isSvg ? buffer : await sharp(buffer).rotate().resize({ width: 1600, withoutEnlargement: true }).webp({ quality: 82 }).toBuffer()

  const { adminClient } = access
  const { error } = await adminClient.storage.from(bucket).upload(fileName, uploadBuffer, { contentType: isSvg ? 'image/svg+xml' : 'image/webp', upsert: false })
