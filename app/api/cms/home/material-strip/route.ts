...
export async function POST(request: Request) {
  ...
  const items = body.items.filter((item: MaterialItem) => {
    return (
  ...
  const { adminClient } = access
  const { error: deleteError } = await adminClient.from('material_strip_items').delete().gte('sort_order', 0)
  if (deleteError) return NextResponse.json({ error: deleteError.message }, { status: 500 })

  if (items.length > 0) {
    const { error: insertError } = await adminClient.from('material_strip_items').insert(items)
