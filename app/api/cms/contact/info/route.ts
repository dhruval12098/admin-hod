...
export async function GET(request: Request) {
  ...
  const { data, error } = await adminClient.from('contact_info').select('id, sort_order, label, value, note, href, icon_path').order('sort_order', { ascending: true })
...
export async function POST(request: Request) {
  ...
  const { adminClient } = access
  const { error: deleteError } = await adminClient.from('contact_info').delete().gte('sort_order', 0)
  if (deleteError) return NextResponse.json({ error: deleteError.message }, { status: 500 })
  const rows = body.items
    .filter((item: any) => typeof item.label === 'string' && typeof item.value === 'string' && typeof item.note === 'string')
  ...
  if (rows.length > 0) {
    const { error: insertError } = await adminClient.from('contact_info').insert(rows)
