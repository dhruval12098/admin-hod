...
export async function POST(request: Request) {
  ...
  const fileName = `category-grid-posters/${safeSlug}/${crypto.randomUUID()}.webp`
  ...
  const { error: uploadError } = await access.adminClient.storage.from(bucket).upload(fileName, uploadBuffer, {
